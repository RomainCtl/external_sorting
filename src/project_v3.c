...
void projectV3_sortFiles(unsigned long nb_split, const char ** filenames, const char ** filenames_sort){
  ...
  struct arg_sort_v3 args[nb_split];
  ...
  for(cpt = 0; cpt < nb_split; ++cpt){
    ...
    thr[cpt] = 1;
    if (pthread_create(&thr[cpt], NULL, v3_sortFiles, (void *) &args[cpt]) != 0) {
...
void projectV3_combMerge(unsigned long nb_split, const char ** filenames_sort, const char * o_file){
  ...
  struct arg_merge_v3 args[NB_THR];
  ...
  for(unsigned long i = 0; i < NB_THR ; ++i){
    ...
    thr[i] = 1;
    if (pthread_create(&thr[i], NULL, v3_mergeFiles, (void *) &args[i]) != 0) {
        fprintf ( stderr , "Erreur dans pthread_create %ld\n", i);
        exit (EXIT_FAILURE);
    }
  }
